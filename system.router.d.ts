...
declare module '@system.router' {
  interface Router {
     ...
     * 
     */
    replace(OBJECT: ReplaceOBJECT): void;
  ...
  /**
   * 
   * @param path 返回目标页面的路径，可以是以下几种取值：不传该参数，返回上一页面以"/"开头的应用内已打开页面的路径；例：/about。特殊的,如果 path 的值是"/",则跳转到页面名称为"/"的页,没有则跳转到首页注意点：path 需要是以"/"开头的当前应用已经打开的页面路径，否则均视为无效参数，返回上一页面若根据 path 未匹配到已经打开的页面，返回上一页面若根据 path 参数匹配到多个页面，返回至最后打开的页面
   */
   ...
   * @param params 跳转时需要传递的数据，参数可以在页面中通过this.param1的方式使用，param1 为 json 中的参数名，param1 对应的值会统一转换为 String 类型
   */
   ...
   * @param params 跳转时需要传递的数据，跳转到短信发送页面时，可以通过body（1040+）插入短信内容；跳转到快应用页面时，参数可以在页面中通过this.param1的方式使用，param1 为 json 中的参数名，param1 对应的值会统一转换为 String 类型
   */
   ...
   * @param body 跳转时短信发送页面时携带的短信内容。
   * @param ___PARAMLAUNCH_FLAG\__ 快应用启动参数，目前仅支持"clearTask"，在启动目标页面时会清除除此页面外的其他页面。详见页面启动模式
   */
  interface PushOBJECTParams {
   body: String;
   '___PARAMLAUNCH_FLAG\__': String;
  }
