...
declare module '@system.record' {
  ...
  /**
   * 
   * @param duration 录音时长，单位为 ms。如果 duration 为有效值将在达到指定值时停止录音
   * @param sampleRate 采样率。不同的音频格式所支持的采样率范围不同。对于 aac 格式，默认设置为 8000，建议使用 8000/16000/44100
   * @param numberOfChannels 录音通道数，有效值 1/2
   * @param encodeBitRate 编码码率。编码码率的取值与采样率和音频格式有关。对 aac 格式，建议按照下表中取值范围来选择编码码率
   * @param format 音频格式，有效值 3gpp/amr_nb/aac。缺省为 3gpp
   * @param success 成功回调
   * @param fail 失败回调
